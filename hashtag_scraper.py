...
class InstagramPost:
    def __init__(self, post_id, code, user="", caption="", display_src=None, is_video=False, created_at=None):
        """
...
class HashTagSearch(metaclass=ABCMeta):
    ...
    @staticmethod
    def extract_likes(node):
        if len(node['edge_liked_by']['edge']) > 0:
            return node['edge_liked_by']['edges'][0]['node']['count']
        else:
            return None
...
if __name__ == '__main__':
    ...
    HashTagSearchExample().extract_recent_tag("rocknrolltroll")
